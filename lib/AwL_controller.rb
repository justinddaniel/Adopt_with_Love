...
class AwLController
  ...
  def petselection
    ...
    if input == 'exit' #Users probably will want a way to get out of this sectin and return to where they can choose a different pet to look at.
      self.summon
    elsif  input.to_i > 0 #I found it necessary to specify that input.to_i method makes an integer of greater than 1 because I found that if text
    ...
    puts peturl #This is the end of the interaction where the user can simply click on the weblink the interface puts out to go directly to the
